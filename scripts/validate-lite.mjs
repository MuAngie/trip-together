...
function readJson(relative) {
  const rootFile = path.isAbsolute(relative) ? relative : path.join(ROOT, relative);
  const staticFile = path.isAbsolute(relative) ? relative : path.join(STATIC_ROOT, relative);
  const file = fs.existsSync(rootFile) ? rootFile : staticFile;
  try {
...
function exists(relative) {
  return fs.existsSync(path.join(ROOT, relative)) || fs.existsSync(path.join(STATIC_ROOT, relative));
}
...
if (config && trip) {
  ...
  if (config.persistence?.mode === "d1") {
    ...
    const allowedCollections = new Set(["todos", "tickets", "ledger", "shopping"]);
