...
const MANIFEST_PATH = "dist/assets/maps/templates/manifest.json";
...
if (config.modules?.overview === false) {
  ...
  for (const region of regions) await fs.access(path.resolve(path.dirname(outPath), region.baseImage));
  const output = {
    ...tripData,
    config,
    map: mapData,
