(() => {
  ...
  function referenceMoney(amount, currency) {
    if (!Number.isFinite(Number(amount))) return "金额待补充";
    try {
      return new Intl.NumberFormat("zh-CN", {
        style: "currency",
  ...
  function referenceDate(date) {
    ...
    return parts.length === 3 ? `${Number(parts[1])}月${Number(parts[2])}日` : "日期待补充";
  }

  function renderKnownCosts() {
    const trip = window.TRAVEL_PLAN_DATA || {};
    const bookings = (trip.bookingsAndTickets || [])
      .filter((item) => Number.isFinite(Number(item.amount)))
      .sort((first, second) => String(first.date || "9999-12-31").localeCompare(String(second.date || "9999-12-31")));
    const fees = (trip.feesAndTaxes || []).filter((item) => Number.isFinite(Number(item.referencePrice?.amount)));
    const count = bookings.length + fees.length;
    if (!count) return "";
    ...
    return `
      <section class="ledger-known-costs" aria-labelledby="ledger-known-costs-title">
        ...
        <div class="ledger-known-cost-list">
          ...
          ${fees.map((item) => `
            <div class="ledger-known-cost-row">
              <span><small>收费标准 · ${escapeHtml(item.referencePrice.unit || "")}</small><strong>${escapeHtml(item.title)}</strong></span>
